...
def discover_files(src: Path, exts: tuple[str, ...], regex: Optional[re.Pattern]) -> list[Path]:
    """rglob, filtered by lowercased extension AND optional full-path regex; sorted."""
...
def convert_one(
    ...
    try:
        ...
        _run(["exiftool", "-overwrite_original_in_place", "-tagsFromFile", str(f), str(dst)])
...
@click.option("--regex", default=None, help="Optional extra full-path regex filter (ANDed with --ext)")
